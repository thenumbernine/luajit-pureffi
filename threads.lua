...
struct {
	...
	volatile int updateWork;	// set when :setwork was called on this thread
	...
	function threads.new_pool(worker_func, num_threads, workData)
		...
		local persistent_worker = function(shared_ptr)
			...
			while true do
				...
				-- NOTICE not calling ':setwork' mid-iterate, i.e. between a ':ready' and ':wait'
				if ctrl.updateWork ~= 0 then
					work = threads.pointer_decode(ctrl.work_data, ctrl.work_data_len)
					ctrl.updateWork = 0
				end
